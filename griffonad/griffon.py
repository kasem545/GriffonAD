...
def _main():
    ...
    parser.add_argument(
        "--groups", action="store_true", help="List groups and members (recursively)"
    )
    parser.add_argument("--ous", action="store_true", help="List OUs (recursively)")
    parser.add_argument(
        "--members",
        action="store_true",
        help="Display all [sub]members of groups/ous (with --groups / --ous)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sep", type=str, default=":", help="Separator string in the owned file"
    )
    parser.add_argument("--save-compiled", type=str, metavar="FILE")
    parser.add_argument(
        "--select", type=str, metavar="STARTSWITH", help="Filter targets/ous/groups"
    )
    ...
    arg_paths.add_argument(
        "--fromv",
        action="store_true",
        help="Paths from vulnerable users (NP users (only unprotected users), password not required,  and kerberoastable users)",
    )
    arg_paths.add_argument(
        "--rights",
        action="store_true",
        help="With --fromo or --fromv, display rights instead of actions",
    )
    arg_paths.add_argument(
        "--da", action="store_true", help="Only paths to domain admin"
    )
    arg_paths.add_argument(
        "--score-paths",
        action="store_true",
    ...
    arg_paths.add_argument(
        "--from", type=str, metavar="USER", help="Get paths from this user"
    )
    arg_paths.add_argument(
        "--opt", type=str, default="", metavar="OPT1,OPT2,...", help="Custom flags"
    )
    parser.add_argument(
        "-f",
        "--no-follow",
    ...
    arg_script.add_argument(
        "-s",
        "--script",
        metavar="N",
        type=str,
        help="Generate commands for a given path. N is the line number",
    )
    arg_script.add_argument("--dc-ip", type=str, default="DC_IP", metavar="DC_IP")
    arg_script.add_argument(
        "--cleanup",
        action="store_true",
        help="Append rollback checklist for generated actions",
    )
    arg_script.add_argument(
        "--detection",
        action="store_true",
        help="Append detection metadata (event IDs and safer alternatives)",
    )
